...
function getUserType(assignment){
	...
	setCookie('UserType',UserType,10*365,'/');
	return(UserType);
}
function getUserID(){
	var UserIDCookieValue = getCookie("UserID");
	var MNGidCookieValue = getCookie("MNGID"); //Emeta Id
	var jSessoinIDCookieValue = getCookie("JSESSIONID");
	if (MNGidCookieValue != ''){
		setCookie('UserID',"R:"+MNGidCookieValue,10*365,'/');
		return ("R:"+MNGidCookieValue);
	}else{
		if (UserIDCookieValue != ''){
			setCookie('UserID',UserIDCookieValue,10*365,'/');
			return UserIDCookieValue;
		}else{
			setCookie('UserID',jSessoinIDCookieValue,10*365,'/');
